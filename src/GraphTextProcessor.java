...
public class GraphTextProcessor {
    ...
    public static String calcShortestPath(String word1, String word2) {
        ...
        if (word2 != null) word2 = word2.toLowerCase();
        ...
        if (word2 == null || word2.isEmpty()) {
            ...
            for (String target : allNodes) {
                ...
                if (distances.get(target) == Integer.MAX_VALUE) {
                    ...
                    sb.append("→ ").append(target).append(" (length = ").append(distances.get(target)).append("):\n");
                    List<List<String>> spaths = paths.get(target);
                    int idx = 1;
                    for (List<String> path : spaths) {
                        sb.append("   Path ").append(idx++).append(": ").append(String.join(" -> ", path)).append("\n");
                    }
                }
            }
        } else {
    ...
    public static Map<String, Double> calculatePageRank() {
        ...
        for (String word : wordFrequencies.keySet()) {
            double tf = (double) wordFrequencies.get(word) / totalFrequency;
            pageRank.put(word, 1.0 / totalWords + tf); // 基础值加上词频权重1

            totalInitialPR += pageRank.get(word); // 累加初始PR值
            outDegree.put(word, graph.get(word).size()); // 计算每个节点的出度
        }

        // 3. 归一化初始 PR 值，使其总和为 1
        for (String word : pageRank.keySet()) {
            pageRank.put(word, pageRank.get(word) / totalInitialPR); // 归一化处理
        }
        ...
        for (int i = 0; i < maxIterations; i++) {
            ...
            for (String word : graph.keySet()) {
                ...
                for (String neighbor : graph.keySet()) {
                    if (graph.get(neighbor).containsKey(word)) {
        ...
        try {
            ...
            for (Map.Entry<String, Map<String, Integer>> entry : graph.entrySet()) {
                ...
                for (Map.Entry<String, Integer> edge : entry.getValue().entrySet()) {
                    ...
                    dotFileContent.append(String.format("    \"%s\" -> \"%s\" [label=%d];\n", node, neighbor, edge.getValue()));
                }
            }
            ...
            ProcessBuilder processBuilder = new ProcessBuilder(dotPath, "-Tpng", tempFile.getAbsolutePath(), "-o", fileName);
